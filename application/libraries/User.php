...
class User {
	...
	*/
	...
	/**
	* Get Program - return the logged user Program.
	* 
	* @return string
	*/
	function get_program(){
		if (isset($this->user_data->program))
			return $this->user_data->program;
		else
			return false;
	}
	...
	*/
	function update_pw($new_pw){
		...
		// update the database
		$this->CI->db->where('id', $this->user_data->id);
		$sts = $this->CI->db->update('users', array('password'=>$new_pw, 'active'=>1));
	...
	*/
	...
	function reset_pw($new_pw,$id){
		...
		$sts = $this->CI->db->update('users', array('password'=>$new_pw, 'active'=>2));
