...
class User_manager {
	...
	* @author Waldir Bertazzi Junior
	**/
	function save_user($program,$full_name, $login, $password, $email, $active = 1, $permissions) {        
		...
		if( ! $this->login_exists($login) && $full_name!= "") {
			...
			if ( $this->CI->db->insert('users', array('program'=>$program,'name'=>$full_name, 'login'=>$login, 'email'=>$email, 'password'=>$hashed_password, 'active'=>$active )) ) {
